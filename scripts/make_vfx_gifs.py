#!/usr/bin/env python3
"""记忆时空 · 动效预览渲染器 v2
- 逻辑画布 320x180（放大 3 倍 → 960x540），像素更细、训练木人清晰人形
- GIF：20ms/帧 = 50fps（浏览器上限）
- MP4：120fps（高帧率，手机播放最顺滑）
"""
...
# ================= 训练木人（清晰人形：斗笠/头/躯干/双臂/双腿） =================
def draw_dummy(im, cx, cy, tilt=0.0, flash=0.0):
    """cy = 地面线 y。tilt：受击后仰位移（像素）。flash 0~1 受击闪白。"""
    d = ImageDraw.Draw(im)
    t = int(round(tilt))
    f = flash
    # 地面阴影
    d.ellipse([cx - 18, cy - 3, cx + 18, cy + 1], fill=(8, 7, 5))
    # 木桩底座
    d.rectangle([cx - 9, cy - 4, cx + 9, cy], fill=WOOD_D, outline=OUTL)
    # 主木桩
    d.rectangle([cx - 4, cy - 96, cx + 4, cy - 4], fill=WOOD, outline=OUTL)
    d.rectangle([cx - 5, cy - 96, cx - 4, cy - 4], fill=WOOD_D)
    # ---- 双腿（木桩腿，微 A 形） ----
    lg = WOOD_D if f < 0.5 else CREAM
    d.line([cx - 3, cy - 30, cx - 7, cy - 6], fill=lg, width=3)
    d.line([cx + 3, cy - 30, cx + 7, cy - 6], fill=lg, width=3)
    # 脚（草团）
    fc = STRAW_D if f < 0.5 else WHITE
    d.rectangle([cx - 11, cy - 7, cx - 3, cy - 4], fill=fc)
    d.rectangle([cx + 3, cy - 7, cx + 11, cy - 4], fill=fc)
    # ---- 躯干（草人上身，受击后仰） ----
    bx = cx + t
    tc = STRAW if f < 0.5 else WHITE
    tm = STRAW_M if f < 0.5 else CREAM
    td = STRAW_D if f < 0.5 else CREAM
    d.rectangle([bx - 10, cy - 58, bx + 10, cy - 30], fill=tc, outline=OUTL)
    # 草绳横纹
    d.rectangle([bx - 8, cy - 52, bx + 8, cy - 50], fill=tm)
    d.rectangle([bx - 8, cy - 44, bx + 8, cy - 42], fill=tm)
    # 胸口中字（认得出是"人"）
    d.line([bx - 3, cy - 56, bx, cy - 51], fill=td, width=1)
    d.line([bx + 3, cy - 56, bx, cy - 51], fill=td, width=1)
    d.line([bx - 4, cy - 47, bx + 4, cy - 47], fill=td, width=1)
    # ---- 双臂（横木臂） ----
    arm = WOOD if f < 0.5 else WHITE
    d.line([bx - 16, cy - 50, bx - 9, cy - 50], fill=arm, width=4)
    d.line([bx + 9, cy - 50, bx + 16, cy - 50], fill=arm, width=4)
    # 手（草团）
    hc = STRAW_D if f < 0.5 else WHITE
    d.rectangle([bx - 19, cy - 52, bx - 15, cy - 48], fill=hc)
    d.rectangle([bx + 15, cy - 52, bx + 19, cy - 48], fill=hc)
    # ---- 头 + 斗笠 ----
    hx = bx
    hy = cy - 74
    hd = STRAW if f < 0.5 else WHITE
    d.rectangle([hx - 7, hy - 8, hx + 7, hy + 4], fill=hd, outline=OUTL)
    # 红布条（额带）
    d.rectangle([hx - 7, hy - 4, hx + 7, hy - 2], fill=REDBAND if f < 0.5 else CREAM)
    # 眼睛
    d.rectangle([hx - 4, hy + 1, hx - 2, hy + 3], fill=INK if f < 0.5 else CREAM)
    d.rectangle([hx + 2, hy + 1, hx + 4, hy + 3], fill=INK if f < 0.5 else CREAM)
    # 斗笠（草帽锥形）
    hp = HAT if f < 0.5 else WHITE
    hp_d = HAT_D if f < 0.5 else CREAM
    d.rectangle([hx - 10, hy - 11, hx + 10, hy - 9], fill=hp_d)
    d.polygon([(hx - 8, hy - 10), (hx + 8, hy - 10), (hx, hy - 20)], fill=hp, outline=OUTL)
    d.line([(hx - 4, hy - 13), (hx, hy - 19)], fill=hp_d)
    d.line([(hx + 4, hy - 13), (hx, hy - 19)], fill=hp_d)
...
def draw_slash(im, cx, cy, t, dur_ms, heavy):
    ...
    sx = cx - 34 - int(p * 14)
    sy = cy - 62
    d.rectangle([sx, sy, sx + 10, sy + 3], fill=c)
    d.rectangle([sx + 10, sy + 1, sx + 16, sy + 4], fill=c)
    d.rectangle([sx + 2, sy + 4, sx + 8, sy + 7], fill=c)
    if heavy:
        d.rectangle([sx - 6, sy - 4, sx + 2, sy + 9], fill=mix(YELLOW, a * 0.7))
        d.rectangle([sx + 12, sy - 3, sx + 20, sy + 2], fill=mix(YELLOW, a * 0.6))
...
def base_scene():
    ...
    # 地面
    d.rectangle([8, 118, LW - 9, LH - 9], fill=(26, 22, 15))
    for x in range(16, LW - 8, 12):
        d.line([(x, 118), (x, LH - 9)], fill=GRID)
    for y in range(124, LH - 9, 12):
        d.line([(8, y), (LW - 9, y)], fill=GRID)
    return im
...
def combat_frames(fps, seconds=3.6):
    ...
    tilt = 0.0
    flash = 0.0
    slashes = []            # [t_ms, dur_ms, heavy]
    parts = []              # [x,y,vx,vy,t,life,color]
    nums = []               # [x,y,t,life,dmg,crit,color] (速度由公式算)
    ...
    for f in range(N):
        ...
        for (at, heavy, stopms) in attacks_ms:
            if abs(t - at) < dt / 2 + 0.01:
                ...
                if heavy:
                    cx_d = -12.0; tilt = -4.0; shake_mag = 3.0; shake_t = 200.0
                    parts += [[cx + random.uniform(-6, 6), cy - 40 + random.uniform(0, 6),
                               random.uniform(-60, 60), random.uniform(-90, -50),
                               0.0, 500.0 + random.uniform(0, 200),
                               [CREAM, STRAW_M, GOLD, WOOD][i % 4]] for i in range(12)]
                    nums.append([cx + random.uniform(-4, 4), cy - 78, 0.0, 900.0,
                                 26 + random.randint(0, 8), True, YELLOW])
                else:
                    cx_d = -5.0; tilt = -2.0; shake_mag = 1.6; shake_t = 160.0
                    parts += [[cx + random.uniform(-4, 4), cy - 36 + random.uniform(0, 4),
                               random.uniform(-50, 50), random.uniform(-70, -40),
                               0.0, 400.0 + random.uniform(0, 150),
                               [CREAM, STRAW_M, GOLD, WOOD][i % 4]] for i in range(5)]
                    crit = random.random() < 0.2
                    nums.append([cx + random.uniform(-4, 4), cy - 78, 0.0, 750.0,
        ...
        for p in parts:
            ...
            a = max(0.0, 1.0 - pn)
            c = mix(p[6], a)
            sz = 1 if int(p[4] / 60) % 3 else 2
            d.rectangle([p[0], p[1], p[0] + sz, p[1] + sz], fill=c)
        for n in nums:
            pn = n[2] / n[3]
            bounce = abs(math.sin(n[2] / 1000.0 * math.pi * 7.2)) * -9.0 * (1.0 - pn * 0.5)
            rise = -30.0 * (n[2] / 1000.0)
...
def glitch_frames(fps):
    ...
    for f in range(N):
        ...
        for y in range(0, LH, band):
            off = int(random.uniform(-1, 1) * 5 * k)
        ...
        if random.random() < 0.6 * k:
            ...
            d.rectangle([x, y, min(LW, x + random.randint(6, 60)), y + 1], fill=RED)
        if random.random() < 0.6 * k:
            x, y = random.randint(0, LW - 1), random.randint(0, LH - 1)
            d.rectangle([x, y, min(LW, x + random.randint(6, 60)), y + 1], fill=BLUE)
        frames.append(im)
    return frames
...
def crack_frames(fps):
    ...
    for f in range(N):
        ...
        segs = [(124, 36), (136, 56), (128, 76), (144, 96), (136, 116), (152, 136)]
        for i in range(len(segs) - 1):
            x0, y0 = segs[i]; x1, y1 = segs[i + 1]
            wd = 1 + int(pulse * 4)
            c = mix((150, 178, 200), 0.45 + pulse * 0.55)
            d.line([(x0, y0), (x1, y1)], fill=c, width=wd)
        d.line([(136, 56), (116, 50)], fill=mix((120, 140, 160), 0.3 + pulse * 0.5), width=1)
        d.line([(128, 76), (160, 72)], fill=mix((120, 140, 160), 0.3 + pulse * 0.5), width=1)
        d.line([(136, 116), (112, 124)], fill=mix((120, 140, 160), 0.3 + pulse * 0.5), width=1)
        if random.random() < 0.25 + pulse * 0.4:
            for _ in range(10):
                x = random.randint(100, 168); y = random.randint(24, 148)
                d.rectangle([x, y, x, y], fill=random.choice([RED, WHITE, CREAM]))
        for _ in range(4):
            if random.random() < pulse:
                x = random.randint(108, 164); y = random.randint(28, 144)
