// ============ 渲染器：瓦片分块 / 光影 / 小地图 / 镜头 ============
import { Application, Container, Graphics, Sprite, Texture, RenderTexture } from 'pixi.js';
import { World, T, Z, TILE, W, H, ZONES, isSolidTile } from '../world/mapgen';
import { gget, clamp, lerp } from '../core/util';

function hash2(x: number, y: number, salt = 0): number {
  let h = (x * 374761393 + y * 668265263 + salt * 2246822519) | 0;
  h = (h ^ (h >>> 13)) * 1274126177 | 0;
  return ((h ^ (h >>> 16)) >>> 0) / 4294967296;
}

function paintDecor(g: Graphics, id: number, x: number, y: number, r: number) {
  const r1 = hash2(x, y, 1), r2 = hash2(x, y, 2), r3 = hash2(x, y, 3);
  const cx = x * TILE + r1 * 28 + 2, cy = y * TILE + r2 * 28 + 2;
  if (id === T.GRASS) {
    if (r3 < 0.7) g.circle(cx, cy, 0.8 + r2 * 1.4).fill({ color: r1 < .5 ? 0x5c7c42 : 0x7da05a, alpha: .55 });
    if (r3 < 0.22) { g.moveTo(cx, cy + 3).lineTo(cx - 1.5, cy - 3).moveTo(cx, cy + 3).lineTo(cx + 1.5, cy - 3).moveTo(cx, cy + 3).lineTo(cx, cy - 4).stroke({ width: 1, color: 0x3f6230, alpha: .8 }); }
  } else if (id === T.ROAD) {
    if (r3 < 0.5) g.circle(cx, cy, 0.7 + r1 * 1.1).fill({ color: 0x4a525e, alpha: .5 });
    if (r3 < 0.1) { g.moveTo(cx, cy).lineTo(cx + (r2 - .5) * 18, cy + (r1 - .5) * 18).stroke({ width: 1, color: 0x3c434e, alpha: .7 }); }
    if (r2 > 0.96) g.circle(cx, cy, 3).fill({ color: 0x424a55, alpha: .6 });
  } else if (id === T.PLAZA) {
    g.moveTo(x * TILE, y * TILE + 1).lineTo(x * TILE + TILE, y * TILE + 1).stroke({ width: 1, color: 0x998f78, alpha: .5 });
    g.moveTo(x * TILE + 1, y * TILE).lineTo(x * TILE + 1, y * TILE + TILE).stroke({ width: 1, color: 0x998f78, alpha: .5 });
    if (r3 < 0.24) g.circle(cx, cy, 1 + r1 * 2).fill({ color: 0x8e8570, alpha: .5 });
  } else if (id === T.YARD) {
    if (r3 < 0.5) g.circle(cx, cy, 0.8 + r2).fill({ color: 0x97805a, alpha: .6 });
    if (r3 < 0.12) { g.moveTo(cx, cy + 4).lineTo(cx, cy - 4).stroke({ width: 1, color: 0x7a8c4a, alpha: .7 }); }
  } else if (id === T.RES_F) {
    g.moveTo(x * TILE, y * TILE + 10).lineTo(x * TILE + TILE, y * TILE + 10).stroke({ width: 1, color: 0x9a7848, alpha: .55 });
    g.moveTo(x * TILE, y * TILE + 21).lineTo(x * TILE + TILE, y * TILE + 21).stroke({ width: 1, color: 0x9a7848, alpha: .55 });
    if (r3 < 0.2) g.circle(cx, cy, 1).fill({ color: 0x8a6a40, alpha: .5 });
  } else if (id === T.HOS_F) {
    g.moveTo(x * TILE + 16, y * TILE).lineTo(x * TILE + 16, y * TILE + TILE).stroke({ width: 1, color: 0xa8bfc4, alpha: .5 });
    g.moveTo(x * TILE, y * TILE + 16).lineTo(x * TILE + TILE, y * TILE + 16).stroke({ width: 1, color: 0xa8bfc4, alpha: .5 });
    if (r3 < 0.12) g.circle(cx, cy, 1.4).fill({ color: 0x9ab4ba, alpha: .4 });
  } else if (id === T.WH_F) {
    g.moveTo(x * TILE, y * TILE + 8).lineTo(x * TILE + TILE, y * TILE + 8).stroke({ width: 1, color: 0x76796f, alpha: .5 });
    if (r3 < 0.2) { g.moveTo(cx, cy).lineTo(cx + 12, cy + 4).stroke({ width: 1, color: 0x6d7066, alpha: .6 }); }
  } else if (id === T.MET_F) {
    g.moveTo(x * TILE + 16, y * TILE).lineTo(x * TILE + 16, y * TILE + TILE).stroke({ width: 1, color: 0x24444f, alpha: .6 });
    g.moveTo(x * TILE, y * TILE + 16).lineTo(x * TILE + TILE, y * TILE + 16).stroke({ width: 1, color: 0x24444f, alpha: .6 });
    if (r3 < 0.3) g.ellipse(cx, cy, 4, 2).fill({ color: 0x1f3944, alpha: .35 });
  } else if (id === T.RAD_F) {
    g.moveTo(x * TILE + 16, y * TILE).lineTo(x * TILE + 16, y * TILE + TILE).stroke({ width: 1, color: 0x51445c, alpha: .6 });
    if (r3 < 0.2) g.circle(cx, cy, 1.2).fill({ color: 0x4a3e54, alpha: .5 });
  } else if (id === T.CEL_F) {
    if (r3 < 0.5) g.circle(cx, cy, 1 + r1 * 1.6).fill({ color: 0x5d4f3e, alpha: .6 });
    if (r3 < 0.15) g.moveTo(cx, cy).lineTo(cx + 10, cy + 8).stroke({ width: 1, color: 0x544836, alpha: .7 });
  }
  // 墙逻辑
  if (id >= 11 && id <= 21 && id % 2 === 1) {
    // 顶部高光
    g.moveTo(x * TILE + 1.5, y * TILE + 1.5).lineTo(x * TILE + TILE - 1.5, y * TILE + 1.5).stroke({ width: 2, color: 0xffffff, alpha: .1 });
  }
}

function tileBaseColor(id: number): number {
  switch (id) {
    case T.GRASS: return 0x6f8f52;
    case T.ROAD: return 0x5a6472;
    case T.PLAZA: return 0xb3a88f;
    case T.YARD: return 0xb09a6a;
    case T.TRACK: return 0x2b3238;
    case T.RES_F: return 0xc8a06a; case T.RES_W: return 0xe6d5b0;
    case T.HOS_F: return 0xcfe0e4; case T.HOS_W: return 0xdfe9ec;
    case T.WH_F: return 0x8f9287; case T.WH_W: return 0xb06a40;
    case T.MET_F: return 0x2f5a66; case T.MET_W: return 0x3d4a58;
    case T.RAD_F: return 0x6a5a72; case T.RAD_W: return 0x55455e;
    case T.CEL_F: return 0x7a6a52; case T.CEL_W: return 0x4a3f38;
    default: return 0x888888;
  }
}

export class RendererCtx {
  tileLayer = new Container();      // 静态地面
  objLayer = new Container();       // 道具/实体（按 y 排序）
  fxLayer = new Container();        // 粒子/光效
  private chunks: Sprite[] = [];
  private lightRT!: RenderTexture;
  private lightC = new Container();
  private overlay!: Sprite;
  private gradTex!: Texture;
  private coneTex!: Texture;
  private camera = { x: 0, y: 0, shake: 0, shakeX: 0, shakeY: 0 };

  constructor(private app: Application, private world: World) {
    this.buildTiles();
    this.buildLighting();
    this.objLayer.sortableChildren = true;
    this.fxLayer.sortableChildren = true;
    app.stage.addChild(this.tileLayer, this.objLayer, this.fxLayer);
    window.addEventListener('resize', () => this.resizeLight());
    this.resizeLight();
    this.camera.x = world.spawn.x; this.camera.y = world.spawn.y;
  }

  get cam() { return this.camera; }

  // ---------- 瓦片分块 ----------
  private buildTiles() {
    const { grid } = this.world;
    const CS = 16;
    for (let cy = 0; cy < H / CS; cy++) for (let cx = 0; cx < W / CS; cx++) {
      const g = new Graphics();
      for (let y = cy * CS; y < (cy + 1) * CS; y++) for (let x = cx * CS; x < (cx + 1) * CS; x++) {
        const id = gget(grid, x, y);
        const r = hash2(x, y, 7);
        const base = tileBaseColor(id);
        const shade = 1 - Math.floor(r * 3) * 0.035;
        g.rect(x * TILE, y * TILE, TILE, TILE);
        g.fill({ color: Math.max(0, Math.min(0xffffff, Math.floor((base >> 16 & 255) * shade) << 16 | Math.floor((base >> 8 & 255) * shade) << 8 | Math.floor((base & 255) * shade))) });
        paintDecor(g, id, x, y, r);
        // 墙：若下方是地板 → 画墙面 + 阴影
        if (isSolidTile(id) && !isSolidTile(gget(grid, x, y + 1))) {
          g.rect(x * TILE, y * TILE + 18, TILE, 14).fill({ color: 0x000000, alpha: 0.16 });
          g.rect(x * TILE, y * TILE + 16, TILE, 4).fill({ color: 0xffffff, alpha: 0.12 });
        }
        // 墙底线
        if (isSolidTile(id)) {
          g.rect(x * TILE, y * TILE + TILE - 2.5, TILE, 2.5).fill({ color: 0x1e1a16, alpha: .8 });
        }
      }
      const tex = this.app.renderer.generateTexture(g);
      const spr = new Sprite(tex);
      spr.position.set(cx * CS * TILE, cy * CS * TILE);
      this.chunks.push(spr);
      this.tileLayer.addChild(spr);
    }
  }

  // ---------- 光影 ----------
  private buildLighting() {
    const c = document.createElement('canvas'); c.width = 256; c.height = 256;
    const ctx = c.getContext('2d')!;
    const gr = ctx.createRadialGradient(128, 128, 10, 128, 128, 128);
    gr.addColorStop(0, 'rgba(255,255,255,1)');
    gr.addColorStop(0.55, 'rgba(255,255,255,0.75)');
    gr.addColorStop(1, 'rgba(255,255,255,0)');
    ctx.fillStyle = gr; ctx.fillRect(0, 0, 256, 256);
    this.gradTex = Texture.from(c);
    const c2 = document.createElement('canvas'); c2.width = 256; c2.height = 256;
    const ctx2 = c2.getContext('2d')!;
    const g2 = ctx2.createLinearGradient(0, 128, 256, 128);
    g2.addColorStop(0, 'rgba(255,255,255,0)');
    g2.addColorStop(0.3, 'rgba(255,255,255,0.85)');
    g2.addColorStop(0.7, 'rgba(255,255,255,0.85)');
    g2.addColorStop(1, 'rgba(255,255,255,0)');
    ctx2.fillStyle = g2; ctx2.fillRect(0, 0, 256, 256);
    const grd = ctx2.createRadialGradient(128, 128, 20, 128, 128, 128);
    ctx2.globalCompositeOperation = 'destination-in';
    ctx2.fillStyle = grd; ctx2.fillRect(0, 0, 256, 256);
    this.coneTex = Texture.from(c2);
    this.resizeLight();
  }

  private resizeLight() {
    const w = this.app.screen.width, h = this.app.screen.height;
    const old = this.lightRT;
    this.lightRT = RenderTexture.create({ width: Math.max(2, Math.ceil(w)), height: Math.max(2, Math.ceil(h)) });
    if (!this.overlay) {
      this.overlay = new Sprite(this.lightRT);
      this.overlay.zIndex = 100;
      this.app.stage.addChild(this.overlay);
    } else this.overlay.texture = this.lightRT;
    old?.destroy(true);
  }

  renderLight(px: number, py: number, aim: number, dark: number, tint: number,
    lights: { x: number; y: number; r: number; a?: number }[], flashlight: boolean, dt: number) {
    const sx = this.app.screen.width / 2, sy = this.app.screen.height / 2;
    this.lightC.removeChildren();
    const darkRect = new Graphics();
    darkRect.rect(0, 0, this.app.screen.width, this.app.screen.height).fill({ color: tint, alpha: dark });
    this.lightC.addChild(darkRect);
    const add = (wx: number, wy: number, r: number, a: number) => {
      const s = new Sprite(this.gradTex);
      s.position.set(wx - this.camera.x + sx, wy - this.camera.y + sy);
      s.blendMode = 'erase';
      s.scale.set(r / 128 * (a || 1));
      this.lightC.addChild(s);
    };
    // 玩家光
    add(px, py, flashlight ? 300 : 260, 1);
    for (const l of lights) add(l.x, l.y, l.r, l.a ?? 1);
    if (flashlight) {
      const cone = new Sprite(this.coneTex);
      cone.position.set(px - this.camera.x + sx, py - this.camera.y + sy);
      cone.rotation = aim;
      cone.blendMode = 'erase';
      cone.scale.set(3.2, 3.2);
      this.lightC.addChild(cone);
    }
    this.app.renderer.render({ container: this.lightC, target: this.lightRT });
  }

  // ---------- 镜头 ----------
  follow(dt: number, tx: number, ty: number) {
    this.camera.x = lerp(this.camera.x, tx, Math.min(1, dt * 6));
    this.camera.y = lerp(this.camera.y, ty, Math.min(1, dt * 6));
    if (this.camera.shake > 0) {
      this.camera.shake -= dt * 3;
      this.camera.shakeX = (Math.random() - 0.5) * this.camera.shake * 14;
      this.camera.shakeY = (Math.random() - 0.5) * this.camera.shake * 14;
    } else { this.camera.shakeX = 0; this.camera.shakeY = 0; }
    const sx = this.app.screen.width / 2, sy = this.app.screen.height / 2;
    this.tileLayer.position.set(sx - this.camera.x + this.camera.shakeX, sy - this.camera.y + this.camera.shakeY);
    this.objLayer.position.copyFrom(this.tileLayer.position);
    this.fxLayer.position.copyFrom(this.tileLayer.position);
  }

  addShake(n: number) { this.camera.shake = Math.min(1.2, this.camera.shake + n); }
  worldToScreen(x: number, y: number) {
    return { x: x - this.camera.x + this.app.screen.width / 2, y: y - this.camera.y + this.app.screen.height / 2 };
  }
}
